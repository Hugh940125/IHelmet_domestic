...
import com.slinph.ihelmet.ihelmet_domestic.adapter.MyExpandAdapter;
    ...
    private void initContent(int item_number) {
        switch (item_number){
            ...
            case 1:
                ...
                addChild(2,"由于头盔与APP之间采用无线信号连接，受距离，阻隔等环境影响较大，因此会存在断开连接，数据不能及时刷新的情况。但是只要在一开始连接后开始治疗，头盔就会自动保存全部的治疗记录，在下一次与手机连接时自动上传至后台服务器保存。");
                addChild(3,"点击治疗界面下方治疗进度条区域，即可查看当月治疗记录及总治疗次数。通过左右滑动，可以查看相邻月份的治疗记录。");
                break;
            case 2:
                ...
                addChild(0,"激光的功率受环境温度影响较大，温度越高，功率就越低，冬夏季可以相差30%以上。" +
                        "简单来说，激光能量=激光功率*照射时间，要保证治疗效果，就要保证激光能量输出恒定，当激光功率下降时，" +
                        "就要增加照射时间补偿，当激光功率提高时，就要减少照射时间。因此i黑密内置高精度传感器，每次开机检测环境温度，" +
                        "自动决定光照时长，保证输出能量恒定。这是i黑密相对传统激光生发产品独特的优势之一。");
                addChild(1,"不需要每天使用，每2天使用一次即可。毛囊细胞组织吸收激光能量后，需要一定的时间进行一系列生化反应来促进细胞的生长，" +
                        "因此毛囊的发育是一个逐步累积的过程，每2天光照一次完全可以满足细胞生长的需求。");
                addChild(2,"在达到最佳生发效果后，依然需要每周使用两次进行保持，既然已经养成了习惯，就像刷牙一样成为您生活的一部分，呵护好您的头发。");
                addChild(3,"充满电后可以用两次。");
                addChild(4,"在APP连接头盔后，光照界面左上角会出现喇叭标志，点击可以打开/关闭头盔声音提示，头盔声音大小不能改变。");
            ...
            case 4:
                ...
                addChild(1,"当定制的光照方案发回用户APP后，可以在专家建议中查看。此时再次连接头盔，光照模式相对于之前的体验模式会有变化，" +
                        ...
                        "在光照界面上也会出现一个脱发级别的标志（用拉丁字母表示），代表属于此种脱发模式的光照方案。");
                addChild(2,"用户在购买产品后需要通过手机APP注册上传自身报告及照片，我们的专家在后台进行诊断并给出定制的光照方案，" +
                        "在随后的治疗期间，用户还需要每30天上传报告和新的照片。专家根据情况变化会提供指导意见，也可能会调整治疗方案，" +
                        "这就是随诊服务，目前是连续六个月免费。");
